...
The graphic below shows the color for each student group for each indicator from the California `r yr` Dashboard.

```{r}
#| results: asis 
#| echo: false 
http_address <- glue::glue("https://www.caschooldashboard.org/reports/{dist}/{yr}")
cat("[California Dashboard 2023](", http_address, ")")
...
The table below displays which student groups qualified `r dist.name` to be eligible for [Differentiated Assistance](https://www.cde.ca.gov/ta/ac/cm/leaproposedcrit.asp). Moreover, for each student group it shows which Priority Areas and associated indicators met qualification criteria.
...
indicator.list <- dash.mry %>% select(indicator) %>% filter(!str_detect(indicator,"CENSUS")) %>% unique() %>% unlist()
